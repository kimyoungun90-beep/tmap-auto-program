export default async function handler(req, res) {
  ...
  if (!appKey) {
    res.status(200).json({
      ok: false,
      error: 'Vercel 환경변수 TMAP_APP_KEY가 없습니다.'
    });
    return;
  }
  ...
  try {
    ...
    if (!summary || !summary.totalDistance) {
      res.status(200).json({
        ok: false,
        error: 'TMAP 응답에서 거리 정보를 찾지 못했습니다.'
      });
      return;
    }
    ...
    res.status(200).json({
      ok: true,
      distanceKm:
        Math.round((Number(summary.totalDistance) / 1000) * 10) / 10,
  ...
  if (!response.ok) {
    return {
      ...
      error:
        data?.error?.message ||
        data?.message ||
        `TMAP 지오코딩 HTTP ${response.status}`
    };
  }

  const coord =
    data?.coordinateInfo?.coordinate?.[0] ||
    data?.coordinateInfo?.newLatEntr ||
    data?.coordinate?.[0];
...
function extractSummary(data) {
  ...
  if (Array.isArray(data.features)) {
    const first = data.features.find(
